...
## Open for Robin
- ~~Analog Electronics tab reuses the Subjects-picker capture~~ Done the same day: `assets/cap-analog-w.webp` is the live build (1.5.4 b37, dcadf586, built in a throwaway worktree) launched on the Ohmie Loop simulator with `-ohmie.auth.accountChoiceMade YES -qaTopic signals -qaTab 0`, captured with `simctl io screenshot` (raw 1206×2622 kept at `~/.claude/ohmie-site-captures/12-analog-home-1.5.4.png`). The build-33 install on the simulators shows an Ohmie World quest the live app hides — never capture from it.
